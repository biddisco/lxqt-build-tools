...
int main(int argc, char* argv[])
{
  ...
  filename << std::filesystem::path(getenv("HOME")).c_str() << "/.ssh/.wallets-"
           << std::put_time(&tm, "%Y-%m-%d-%H-%M-%S");
  std::filesystem::path p = filename.str();
  out_filename = std::filesystem::absolute(p);

  std::cout << "example command line :\n"
            << "./vanity --prefixes johnb jbjnr johnnyb biddi biddisco olga olgy olgypops olgab "
               "sasha mila milena grox -f 100000 --pika:threads=cores \n\n"
            << "example cleanup line :\n"
            << "wc -l ~/.ssh/.wallets.txt && cat ~/.ssh/.wallets* | sortci | uniq >> temp.wallets "
               "&& mv temp.wallets ~/.ssh/.wallets.txt && wc -l ~/.ssh/.wallets.txt \n\n"
