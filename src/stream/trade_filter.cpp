...
struct print
{
  ...
  template <typename T>
  typename result<print(T)>::type operator()(const T& value) const
  {
  ...
  std::vector<price_type>& price_pipelines)
{
  ...
  auto pipeline = ((ohlc_input | cm_filter(ohlc_candlemaker(res, ohlc_data_resolutions::minute)) |
                     ha_filter(ohlc_heikin_ashi()) | ha_trans(heikin_ashi_transition())) +
...
#define VWMA volume_weighted_moving_average
  auto pipeline = ((((ohlc_input | VWMA(N).f()) + (ohlc_input | VWMA(M).f())) | cross().f()) +
                    (time_input | dummy<double>().f())) |
    time_filt(add_time_filter());
  event_pipelines.push_back(pipeline);
  //
...
void make_MACD(ohlc_input_type& ohlc_input, time_input_type& time_input, int fast, int slow, int O,
  std::vector<event_type>& event_pipelines, std::vector<price_type>& price_pipelines,
  std::vector<price_type>& filter_pipelines)
{
  ...
  price_pipelines.push_back(pipeline3);
  auto pipeline4 = ((ohlc_input | VWMA(slow).f()) + (ohlc_input | VWMA(fast).f())) |
    difference().f() | moving_average(O).f();
