...
namespace pipeline {
  ...
  pipeline::filter<Out, In...>
  operator|(pipeline::filter<Mid, In...> ip, pipeline::filter<Out, Mid> op)
  {
  ...
  {
    pipeline::output<> new_function = [ip, op]() { op(ip()); };
    return new_function;
  }

  // --------------------------------------------------------------
  // take a Joiner and pass it into a filter
  template <typename Joiner, template <typename, typename...> typename Filter, typename Out,
    typename... Args>
  ...
  {
    return for_each_impl(std::forward<Tuple>(tuple), std::forward<Functions>(f),
  ...
  {
    return filter_joiner<Out1, Out2, pipeline::pfunc<Out1>, pipeline::pfunc<Out2>>(
