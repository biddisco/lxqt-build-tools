...
{
  GROX_LOG_TRACE(xrpnet_log, "{:>20} testnet {}", "destructor", testnet());
}
...
void xrpl_network::initialize()
{
  ...
  auto wait_for_init = [this]() {
    ...
    GROX_LOG_TRACE(xrpnet_log, "{:>20} complete", "initialize scope");
...
stream_set xrpl_network::ticker_subscribe(currency_pair const& cp)
{
  ...
  {
    GROX_LOG_TRACE(xrpnet_log, "{:>20} {} subscribed", "subscription", cps);
    return stream_set{};
  }
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "subscribing", cps);
...
{
  GROX_LOG_TRACE(xrpnet_log, "{:>20}", "shutdown start");
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} Resetting ws_orderbook", "shutdown");
  if (ws_orderbook) { ws_orderbook.reset(); }

  GROX_LOG_TRACE(xrpnet_log, "{:>20} Resetting ws_accounts", "shutdown");
...
bool xrpl_network::subscribe_order_book(currency_pair const& cp, bool enable)
{
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} orderbook xrpl: {}", "Subscribing", currency_pair_string(cp));
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "subscribe orderbook", subscription);
...
bool xrpl_network::subscribe_accounts()
{
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} account changes for {}", "Subscribing", addresses);
...
{
  GROX_LOG_TRACE(xrpnet_log, "{:>20} Ticker {}", "Orderbook", currency_pair_string(cp));
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "Orderbook data", data.toStdString());
  ...
  auto process = [abstract_exchange, cp, data]() {
    ...
    try
    {
      ...
      {
        GROX_LOG_TRACE(xrpnet_log, "{:>20}", "ledger_snapshot");
      ...
      {
        GROX_LOG_TRACE(xrpnet_log, "{:>20}", "ledger_transaction");
        dynamic_pointer_cast<xrpl_order_book>(tdata->orderbook_)
      ...
      //
      GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "orderbook callback", currency_pair_string(cp));
...
void xrpl_network::new_account_data_q(xrpl_network* nw, QString qdata)
{
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "Account changes", data);
  ...
  if (startswith(data, "{\"result\":"))
  {
    ...
    GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "Account subscription", data);
  }
  else if (startswith(data, "{\"engine_result\":\"tesSUCCESS\""))
  {
    json jdata = json::parse(data);
    GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "Account changes", jdata.dump(4));
    json adata = jdata["meta"]["AffectedNodes"];
    for (auto const& a : adata)
    {
      //            try {
      GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "AffectedNode", jdata.dump(4));
...
any_bytearray_sender xrpl_network::submit_signed_transaction(std::string&& signed_tx)
{
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "signed_transaction", url);
  ...
  for (auto& w : subscribed_wallets_)
  {
    auto snd =
        ...
        | stdexec::then([this, &w](QByteArray byteArray) {                                 // pika
            ...
            GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "Ledger response", data);
...
void xrpl_network::handle_account_lines(ledger_wallet& w, std::string_view data)
{
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "account lines", jdata.dump(4));
  ...
  for (auto& w : subscribed_wallets_)
  {
    auto snd = stdexec::starts_on(QtStdExec::QThreadScheduler(), get_account_info(w.public_))    //
        | stdexec::then([this, &w](QByteArray byteArray) {
            std::string_view data(byteArray.constData(), byteArray.length());
            // debug : print the response headers and body
            GROX_LOG_TRACE(xrpnet_log, "{:>20} {} {}", "account_info", w.public_, data);
...
void xrpl_network::handle_account_info(ledger_wallet& w, std::string_view data)
{
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "account info", jdata.dump(4));
  //
  ...
  w.compute_ledger_reserve();
  //
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {} {}", "sequence", w.public_, w.sequence_);
  // signal GUI to update
  emit wallet_changed(&w);
}
  ...
  for (auto& w : subscribed_wallets_)
  {
    auto snd =
        ...
        | stdexec::then([this, &w](QByteArray byteArray) {
            std::string_view data(byteArray.constData(), byteArray.length());
            // debug : print the response headers and body
            GROX_LOG_TRACE(xrpnet_log, "{:>20} {} {}", "account_offers", w.public_, data);
...
void xrpl_network::handle_account_offers(ledger_wallet& w, std::string_view data)
{
  ...
  GROX_LOG_TRACE(xrpnet_log, "{:>20} {}", "account offers", jdata.dump(4));
  //
  if (jdata.is_null()) return;
  try
  {
...
void xrpl_network::query_iou_fee(currency_code const& c1)
{
  ...
  auto snd = stdexec::starts_on(QtStdExec::QThreadScheduler(), get_account_info(c1.issuer_))    //
      | stdexec::then([this, c1](QByteArray byteArray) {
          std::string_view data(byteArray.constData(), byteArray.length());
          // debug : print the response headers and body
          GROX_LOG_TRACE(xrpnet_log, "{:>20} {} {}", "query_iou_fee", c1.issuer_, data);
          ...
          if (jdata.contains("TransferRate"))
          {
            ...
            GROX_LOG_TRACE(xrpnet_log, "{:>20} {} {}", "fee %", c1.issuer_, feepercent);
