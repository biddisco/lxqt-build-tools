  ...
  : QDialog()
{
    ...
    connect(
        ui.xrp_nickname, SIGNAL(textChanged(QString)), this, SLOT(enable_ok_button()));
...
password_dialog::password_dialog(const std::array<std::string, 6>& strings)
  : password_dialog()
{
...
void password_dialog::enable_ok_button()
{
    if (ui.api_user->text().isEmpty() || ui.api_key->text().isEmpty() ||
        ui.api_secret->text().isEmpty() || ui.password->text().isEmpty())
    {
        ui.ok_button->setEnabled(false);
        return;
    }
